 ...
 *
 * Processed with
 * indent --verbose -br -brs -i4 -nut --line-length120 --comment-line-length120 --leave-preprocessor-space -npcs
 *
 */
...
#define RES_ID(handle) INT_EL((handle),2)
    ...
    void RS_DBI_setException(Db_Handle * handle, DBI_EXCEPTION exceptionType, int errorNum, const char *errorMsg);
